...
class LaneDetection:
    ...
    high_yellow = np.array([40, 55, 120])
    low_white = np.array([0, 130, 0])
    high_white = np.array([255, 255, 255])
    #birdeye points
    #upper_left, upper_right, lower_left, lower_right
    pts1 = np.float32([[175, 240], [465, 240], [0, 380], [640, 380]]) #for showing
    ...
    def __init__(self):
        ...
        self.img_sub = rospy.Subscriber("/usb_cam/image_raw",Image,self.convertIng)  
    ...
    def steering(self, frame, slidng_list_left, sliding_list_right):
        ...
        if len(slidng_list_left) == 0:
            ...
            left_distance = image_width - slidng_list_left[0][1] #should be positive

        if len(sliding_list_right) == 0:
            right_distance = 0
        else:
            right_distance = image_width - sliding_list_right[0][1] #should be negative
        ...
        self.steering_pub.publish(avg_val*1.4 + dist_steer * 0.03)
