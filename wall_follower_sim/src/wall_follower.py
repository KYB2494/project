...
SHOW_VIS = True
FAN_ANGLE = np.pi/10.0
TARGET_DISTANCE = 1.0
MEDIAN_FILTER_SIZE = 3
KP = 1.0  # distance term
KD = 0.35  # angle term
# KD = 0.5  # angle term
PUBLISH_LINE = True
...
class WallFollower:
    ...
    def __init__(self, direction):
        ...
        self.pub = rospy.Publisher("/drive",
        ...
        if PUBLISH_LINE:
            self.line_pub = rospy.Publisher(
                "/viz/line_fit", PolygonStamped, queue_size=1)
    ...
    def compute_pd_control(self):
        if self.received_data:
            ...
            # rospy.loginfo("wall_dist={}".format(wall_dist))
            rospy.loginfo("dist, theta ={}, {}".format(wall_dist, theta))

            # control proportional to angular error and distance from wall
            distance_term = self.direction_muliplier * \
                KP * (wall_dist - TARGET_DISTANCE)
            angle_term = KD * theta
            control = angle_term + distance_term
            # control = distance_term
            # control = angle_term
    ...
    def lidarCB(self, msg):
        ...
        angles = self.laser_angles[(
            values > msg.range_min) & (values < msg.range_max)]
        ...
        # rospy.loginfo("filtered ={}".format(filtered_ranges))
        ...
        # rospy.loginfo("filtered ranges ={}".format(filtered_ranges))
        # rospy.loginfo("filtered_angles ={}".format(filtered_angles))

        # convert from polar to euclidean coordinate space
        self.xs = filtered_ranges * np.cos(filtered_angles)
        self.ys = filtered_ranges * np.sin(filtered_angles)
    ...
    def viz_loop(self):
        if self.received_data == True:
            ...
            self.viz.laser_filtered.set_data(
                self.filtered_angles, self.filtered_ranges)
